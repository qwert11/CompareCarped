...
begin
  ...
  begin
    ...
    begin
      try
        ...
        on E: Exception do
          begin
            Error('Пустая строка №(%d) ' + E.Message, i);
            Inc(ii);
          end;
      end;
    end;
  ...
  begin
    if TInfoRecord(Self[i]).itsCount < 0 then
      Error('Кол-во записи (%d) меньше нуля', i);
    if TInfoRecord(Self[i]).itsCount = 0 then
      Delete(i)
    else
      Inc(i);
...
const
  ...
  scCount = 'В налич. В нал. шт. В налич. В нал.';
  scLength = 'Ш Ширина';
...
//    // CarpedPosition:
//    newLength := 0;
//    temp.rEnd := TitlePosition.rPosition.rHeadline; // для поиска сначала сетки sg
//    repeat
